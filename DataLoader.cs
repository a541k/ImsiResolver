...
namespace Imsi
{
    public class ImsiResolver : KeyValueRegistrar
    {
        ...
        private List<KeyValuePair<string, string>> LoadBatch(int offset, int limit)
        {
            ...
            string query = "SELECT imsi, phoneNumber FROM imsiinfo LIMIT @limit, @offset";//-----------<<<<<<<<<<query
        ...
        public void AddData(List<KeyValuePair<string,string>> imsiVsPhoneNumbers)
        {
            ...
            {
                conn.Open();
                string insertHeader = "INSERT INTO imsiinfo (imsi, phoneNumber) VALUES \n";
        ...
        public void DeleteData(string imsi)
        {
            ...
            {
                conn.Open();
                string query = "DELETE FROM imsiinfo WHERE imsi = @imsi";
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
